...
function Thunder(options) {
  options = options || {};
  this.lifespan = options.lifespan || Math.round(Math.random()
   * 10 + 10);
  ...
  this.update = function (index, array) {
    this.segments.forEach(s => {(s.direct += s.change) && Math.random(
    ) > 0.96 && (s.change *= -1);});
...
function Text(options) {
  ...
  pool.width = w;
  buffer.fillStyle = '#000000ff';
  buffer.fillRect(0, 0, pool.width, pool.height);

  this.size = options.size || 100;
  this.copy = (options.copy || `webkaizen!`) + ' ';
  this.color = options.color || '#00e1ffff';
